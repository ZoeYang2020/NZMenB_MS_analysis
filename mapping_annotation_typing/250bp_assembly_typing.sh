...
while getopts 'r:d:n:h' opt; do
  case $opt in
    ...
    n) name=$OPTARG ;;
    h) echo "Usage: bash 250bp_trim_assembly_align.sh -r <miseq_reads_directory> -d <designation_directory> -n <sequence ending ie _R1.fastq.gz etc> -h <help>"
        exit
        ;;
  esac
done
...
##de novo assembly
spades.py -k 21,33,55,77,99,127 -t 10 -m 20 --careful --cov-cutoff auto -1 ${miseq_reads_directory}/$read1 -2 ${miseq_reads_directory}/$read2 -o ${spades_directory}/${x}
...
kraken --db /opt/bioinf/kraken/kraken_db/standard ${spades_directory}/${x}more300.fasta | kraken-report --db /opt/bioinf/kraken/kraken_db/standard ${spades_directory}/kraken.output > ${spades_directory}/${x}.kraken
